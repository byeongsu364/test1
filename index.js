...
app.post("/boards", (req, res)=>{
    try {
        ...
        res.status(201).json({message : "게시글 생성완료", boards})
    } catch (error) {
...
app.get("/boards", (req, res)=>{
    try {
        res.status(201).json({message : "불러오기 성공!", boards})
    } catch (error) {
...
app.get("/boards/:id", (req, res) => {
    try {
        const newBoard = Number(req.params.id);

        const index = boards.findIndex(u => u.id === newBoard);
...
app.put("/boards/:id", (req, res) => {
    try {
        ...
        res.status(200).json({ message: "게시글 1개 수정 완료", board: boards[index] });
    } catch (error) {
...
app.delete("/boards/:id", (req, res) => {
    try {
        ...
        res.status(200).json({ message: "게시글 1개 삭제 완료", boards });
    } catch (error) {
...
app.patch("/boards/:id/title", (req, res)=>{
    try {
        ...
        if (index === -1) {
            return res.status(404).json({ message: "게시글 일부 수정중 아이디가 없음" });
        }

        const {title} = req.body

        if(typeof title !== 'string' || title.trim()===""){
            return res.status(400).json({message : "타이틀은 비어있지 않은 문자열이어야 합니다."})
        }
...
app.patch("/boards/:id/content", (req, res)=>{
    try {
        ...
        console.error("게시글 내용 수정 중 오류", error);
        res.status(500).json({ message: "서버 내부 오류 발생" });
    }
})
